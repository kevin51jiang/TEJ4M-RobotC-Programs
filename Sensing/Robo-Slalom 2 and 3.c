...
/*+++++++++++++++++
This code is for a very basic line follower that jitters around a lot. 
It can be improved, but it works well enough to pass the scenarios (no time limits).
The best part is that it can be easily adjusted based on difficulty (move the jitter time up and down).
++++++++++++++++*/
void stopMoving(){
